...
const getAllWorlds = async () => {
  return WorldDataService.getAll()
    .then((res) => {
      worlds.value = res.data.data.data;
    })
    .catch((error) => {
...
async function storeEquipment(){
  ...
  return GameDataService.addEquipment(equipmentData);
}
...
<template>
  ...
  <main>
    <div>
      ...
      <h2 class="text-white mt-10 mb-2 text-2xl">Choose world:</h2>
      <table id="tableone" class="border-separate rounded  border-spacing-2 lg:text-lg mb-10 border-2 border-orange-500 m-auto text-black">
        <thead>
        ...
        <tbody>
            <tr v-for="world in worlds" class="border-2 border-orange-500">
            <td>{{ world.name }}</td>
            <td>{{ world.players }}</td>
            <td>{{ world.age }}</td>
            <td>{{ world.status }}</td>
